...
// DefaultStreamPreviewCfg 返回合理的默认值
...
func NewStreamPreview(cfg StreamPreviewCfg, p types.Platform, replyCtx any, ctx context.Context, transform func(string) string) *streamPreview {
...
func (sp *streamPreview) Freeze() {
	...
	if sp.previewMsgID != nil && !sp.degraded {
		if updater, ok := sp.platform.(types.MessageUpdater); ok {
			text := sp.fullText
...
func (sp *streamPreview) Finish(finalText string) bool {
	...
	updater, ok := sp.platform.(types.MessageUpdater)
	if !ok {
...
func (sp *streamPreview) CanPreview() bool {
	...
	_, ok := sp.platform.(types.MessageUpdater)
	return ok
}
