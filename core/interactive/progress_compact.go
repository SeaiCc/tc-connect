...
type compactProgressWriter struct {
	...
	starter streaming.PreviewStarter
	updater types.MessageUpdater
	handle  any
...
func NewCompactProgressWriter(ctx context.Context, p types.Platform, replyCtx any, agentName string, lang i18n.Language, transform func(string) string) *compactProgressWriter {
	...
	updater, ok := p.(types.MessageUpdater)
	if !ok {
	...
	w.enabled = true
	w.updater = updater
	if starter, ok := p.(streaming.PreviewStarter); ok {
		w.starter = starter
	}
...
func progressStyleForPlatform(p types.Platform) string {
	ps := progressStyleLegacy
	if sp, ok := p.(types.ProgressStyleProvider); ok {
...
func BuildProgressCardPayloadV2(items []ProgressCardEntry, truncated bool, agent string, lang i18n.Language, state ProgressCardState) string {
