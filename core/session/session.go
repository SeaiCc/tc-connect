...
func (s *Session) GetHistory(n int) []types.HistoryEntry {
	s.mu.Lock()
	defer s.mu.Unlock()
	total := len(s.History)
	if n <= 0 || n > total {
...
func (s *Session) AddHistory(role, content string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.History = append(s.History, types.HistoryEntry{
		Role:      role,
		Content:   content,
		Timestamp: time.Now(),
	})
...
type SessionManager struct {
	mu            sync.RWMutex
	sessions      map[string]*Session //session管理器分配id -> Session
	activeSession map[string]string   // 平台侧提供key -> session管理器分配id
...
// 创建session (with Lock)
// userKey为platform侧根据platformName+chatID+userID生成key, activateSession: userKey->id
// id 为sessionManager 计数器分配id, sessions: id -> Session{}
// userSessions: 一个userKey可对应多个 id，但GetOrCreateActive有第一次判断，何时添加第二个id
...
func (sm *SessionManager) saveLocked() {
	...
	for id, s := range sm.sessions {
		...
		snapSessions[id] = &Session{
			...
			History:        append([]types.HistoryEntry(nil), s.History...),
	...
	// 原子写入 sm storePath
	if err := types.AtomicWriteFile(sm.storePath, data, 0o644); err != nil {
