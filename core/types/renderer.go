/*该包应该只包含数据结构定义（type definitions），不包含构建逻辑（construction logic）*/
package types
...
// 外部依赖 Renderer 渲染服务接口
type Renderer interface {
	// 会话管理
	RenderListCard(sessionKey string, page int) (*Card, error)
	RenderCurrentCard(sessionKey string) *Card
	// 状态信息
	RenderStatusCard(sessionKey string, userID string) *Card
	// 历史记录
	RenderHistoryCard(sessionKey string) *Card
	// 模型切换
	RenderModelCard(models []ModelOption, current string) *Card
	// 语言设置
	RenderLangCard() *Card
	// 定时任务
	RenderCronCard(sessionKey string, userID string) *Card
	// 帮助
	RenderHelpCard() *Card
	// 命令管理
	RenderCommandsCard() *Card
	// 技能管理
	RenderSkillsCard() *Card
	// 配置管理
	RenderConfigCard() *Card
	// 别名管理
	RenderAliasCard() *Card
	// 删除模式
	RenderDeleteModeCard(sessionKey string) *Card
	// 用户信息
	RenderWhoamiCard(msg *Message) *Card
	// 平台信息  cmd/platform层依赖
	RenderCardForPlatform(p Platform, card *Card) *Card

	//  		agent/platform层依赖
	RenderOutgoingContentForWorkspace(p Platform, content, workspaceDir string) string
}
