...
func New(opts map[string]any) (types.Platform, error) {
	wsURL, _ := opts["ws_url"].(string)
	if wsURL == "" {
		wsURL = "ws://127.0.0.1:3001"
	}
	token, _ := opts["token"].(string)
	slog.Info("qq: platform created", "ws_url", wsURL, "token", token, "token_len", len(token))
	allowFrom, _ := opts["allow_from"].(string)
	shareSessionInChannel, _ := opts["share_session_in_channel"].(bool)

	types.CheckAllowFrom("qq", allowFrom)
...
func (p *Platform) parseMessage(payload map[string]any, msgType string, groupID int64) (string, []types.ImageAttachment, []types.FileAttachment, *types.AudioAttachment) {
	var textParts []string
	var images []types.ImageAttachment
	var files []types.FileAttachment
	var audio *types.AudioAttachment
		...
		for _, seg := range msg {
			...
			case "image":
				if url, ok := data["url"].(string); ok && url != "" {
					...
					images = append(images, types.ImageAttachment{
						MimeType: mime,
						Data:     imgData,
					})
				}
...
func (p *Platform) SendImage(ctx context.Context, replyCtx any, img types.ImageAttachment) error {
	rctx, ok := replyCtx.(*replyContext)
	if !ok {
...
var _ types.ImageSender = (*Platform)(nil)
...
func (p *Platform) SendFile(ctx context.Context, replyCtx any, file types.FileAttachment) error {
	rctx, ok := replyCtx.(*replyContext)
	if !ok {
...
var _ types.FileSender = (*Platform)(nil)
